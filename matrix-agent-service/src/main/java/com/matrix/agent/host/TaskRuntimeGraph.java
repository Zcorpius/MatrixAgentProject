...
import com.matrix.agent.data.audit.AuditRepositoryTaskAuditSink;
...
import com.matrix.agent.data.memory.MemoryWriterTaskMemoryWriter;
...
import com.matrix.agent.task.ConversationCompressor;
import com.matrix.agent.task.DefaultContextUpdater;
import com.matrix.agent.task.DemoModelGateway;
import com.matrix.agent.task.LlmSummaryProvider;
    ...
    TaskRuntimeGraph(Dependencies values) {
        ...
        AgentRuntimeRepository.AgentEngineFactory engineFactory = gateway -> {
            SummaryProvider summaryProvider = gateway instanceof OnDeviceModelGateway
                    ? null : new LlmSummaryProvider(values.modelClient, values.configStore);
            AgentEngineConfiguration configuration = new AgentEngineConfiguration.Builder()
                    .auditSink(new AuditRepositoryTaskAuditSink(values.auditRepository))
                    ...
                    .taskMemoryWriter(new MemoryWriterTaskMemoryWriter(values.memoryWriter))
                    .build();
